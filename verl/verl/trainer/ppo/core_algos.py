...
@register_adv_est('dae')
def compute_psr_nsr_outcome_advantage(
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    **kwargs,
    ):
    """
    DAE: Debiased Advantage Estimation.
    Assigns fixed advantages: positive samples → +1, negative samples → -1,
    independent of group statistics. This eliminates the spurious amplification
    introduced by GRPO's group-relative normalization in unsupervised settings.
    ...
    """
    scores = token_level_rewards.sum(dim=-1)
    invalid_scores = (scores != 0) & (scores != 1)
    if invalid_scores.any():
        raise ValueError(
            f"DAE expects binary rewards (0 or 1), got: {scores[invalid_scores].tolist()}"
        )
    # positive → +1, negative → -1
    scores = (scores.clone() - 0.5) * 2
    scores = scores.unsqueeze(-1) * response_mask
    return scores, scores
