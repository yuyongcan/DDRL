...
def compute_score(model_response, gt_answer,counter, fast=False):
    model_answer = extract_answer(str(model_response))
    ...
    count = counter[simplify_expression_string(model_answer)]
    if count>=20:
        reward=1.0
    elif count<=2:
        reward=0.1
...
def compute_score_2(model_response, gt_answer, fast=False):
...
def reward_func(
    data_source, solution_str, ground_truth,counter, extra_info=None, sandbox_fusion_url=None, concurrent_semaphore=None
):  
    if counter is not None:
        try:
            res = compute_score(solution_str, str(ground_truth), counter)
        ...
        try:
            res = compute_score_2(solution_str, str(ground_truth))
