...
def compute_advantage(
    ...
    elif adv_estimator == 'dae':
        # DAE: Debiased Advantage Estimation — fixed ±1, no group-relative normalization
        grpo_calculation_mask = data.batch["response_mask"]
        advantages, returns = core_algos.compute_psr_nsr_outcome_advantage(
            token_level_rewards=data.batch["token_level_rewards"],
            response_mask=grpo_calculation_mask,
        )
        data.batch["advantages"] = advantages
...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        _adv_estimator = (
            'dae'
            if self.config.get("ddrl", {}).get("dae", {}).get("enable", False)
            else self.config.algorithm.adv_estimator
        )
        if _adv_estimator == AdvantageEstimator.GAE:
            self.use_critic = True
        elif _adv_estimator in [
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("gen", timing_raw, color="red"):
                        if self.config.get("ttrl", {}).get("enable", False):
                            from verl.trainer.ppo.ttrl_utils import select_top_k_per_prompt, apply_ttrl_gt, apply_ttrl_cnt, select_top_k_per_prompt_cnt
                            # print(batch)
                            ...
                            majority_ratio_arr = batch.non_tensor_batch["majority_ratio_list"]
                            second_ratio_arr = batch.non_tensor_batch['second_ratio_list']
                            if self.config.get("ddrl", {}).get("bcs", {}).get("enable", False):
                                gen_batch_output.non_tensor_batch['count']=np.zeros(len(gen_batch_output), dtype=object)
                                batch , gen_batch_output = apply_ttrl_cnt(batch, gen_batch_output, self.config.ttrl.n_votes_per_prompt, self.tokenizer)
                            print(batch)                            

                            bcs_enabled = self.config.get("ddrl", {}).get("bcs", {}).get("enable", False)
                            gen_batch_output = select_top_k_per_prompt_cnt(
                                gen_batch_output,
                    ...
                    print(batch.non_tensor_batch["majority_ratio_list"])
                    # update critic
                    if self.use_critic:
